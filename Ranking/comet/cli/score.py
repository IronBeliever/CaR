...
def score_command() -> None:
    ...
    if 'score' in cfg.model.lower():
        with open('APE_score_test.csv', encoding="utf-8") as fp:
            ...
            for row in reader:
                ...
                scores.append(row[1])
        data = {"src": sources, "score": scores}

    elif 'instruction' in cfg.model.lower():
        with open(cfg.sources(), encoding="utf-8") as fp:
            content = fp.read()
            sources = content.split('[<next line>]\n')[:-1]

        translations = []
        for path_fr in cfg.translations:
            with open(path_fr(), encoding="utf-8") as fp:
                content = fp.read()
                translations.append(content.split('[<next line>]\n')[:-1])
    ...
    if cfg.gpus > 1:
        ...
        if 'score' not in cfg.model.lower():
            ...
            count = 0       # positive sample number in test set
            count_pos = 0
            count_neg = 0
            for i in range(len(data['src'])):
                if scores[i] == '1':
                    count += 1
                if seg_scores[i] > 0.5 and scores[i] == '1':
                    count_pos += 1      # positive sample and calculate right
                elif seg_scores[i] < 0.5 and scores[i] == '0':
                    count_neg += 1      # negative sample and calculate right
            print('Accuracy in {} test dataset: {:.4f}'.format(len(data['src']), (count_pos + count_neg)/len(data['src'])))
            print('Accuracy in {} positive sample: {:.4f}'.format(count, count_pos / count))
            print('Accuracy in {} nagetive sample: {:.4f}'.format(len(data['src'])-count, count_neg/(len(data['src'])-count) ))

    if 'instruction' in cfg.model.lower() and 'score' not in cfg.model.lower():
        count = 0
        for i in range(len(data[files[0]])):
            if seg_scores[0][i] > seg_scores[1][i]:
                count += 1
        print('Accuracy in test dataset: {:.4f}'.format(count/len(data[files[0]])))
