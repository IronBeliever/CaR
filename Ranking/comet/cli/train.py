...
def read_arguments() -> ArgumentParser:
    ...
    parser.add_subclass_arguments(InstructionMetric, "instruction_metric")
    ...
    parser.add_subclass_arguments(InstructionRankingMetric, "instruction_ranking_metric")
...
def initialize_model(configs):
    ...
    if configs.regression_metric is not None:
    ...
    elif configs.instruction_metric is not None:
        print(
            json.dumps(
                configs.instruction_metric.init_args,
                indent=4,
                default=lambda x: x.__dict__,
            )
        )
        if configs.load_from_checkpoint is not None:
            logger.info(f"Loading weights from {configs.load_from_checkpoint}.")
            model = InstructionMetric.load_from_checkpoint(
                checkpoint_path=configs.load_from_checkpoint,
                strict=configs.strict_load,
                **namespace_to_dict(configs.instruction_metric.init_args),
            )
        else:
            model = InstructionMetric(
                **namespace_to_dict(configs.instruction_metric.init_args)
            )
        ...
        if configs.load_from_checkpoint is not None:
            logger.info(f"Loading weights from {configs.load_from_checkpoint}.")
            # 这个错了吗❌？为什么 ranking_metric 导入的参数是 ReferencelessRegression类的
            model = ReferencelessRegression.load_from_checkpoint(
                checkpoint_path=configs.load_from_checkpoint,
                strict=configs.strict_load,
    ...
    elif configs.instruction_ranking_metric is not None:
        # dumps 将 dict 转为 str 格式
        print(
            json.dumps(
                configs.instruction_ranking_metric.init_args, indent=4, default=lambda x: x.__dict__
            )
        )
        if configs.load_from_checkpoint is not None:
            logger.info(f"Loading weights from {configs.load_from_checkpoint}.")
            model = InstructionRankingMetric.load_from_checkpoint(
                checkpoint_path=configs.load_from_checkpoint,
                strict=configs.strict_load,
                **namespace_to_dict(configs.instruction_ranking_metric.init_args),
            )
        else:
            model = InstructionRankingMetric(**namespace_to_dict(configs.instruction_ranking_metric.init_args))
