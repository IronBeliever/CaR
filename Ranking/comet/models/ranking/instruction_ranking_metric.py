...
r"""
Instruction Ranking Metric
====================
    Instruction Translation Ranking metric was introduced by [Gy QAQ]!
...
class InstructionRankingMetric(CometModel):
    """RankingMetric
    ...
    def loss(self):
        '''
            TripletMarginLoss(a, p, n) = max{ dis(ai, pi) - dis(ai, ni) + margin,0 }
            - margin: 超参
            - p: The norm degree for pairwise distance. Default: 2 (维度？ 二范数？)
        '''
    ...
    def prepare_sample(
        ...
        Returns:
            Model inputs. If stage == 'predict' we will return only the src, mt and ref
                ...
                the input ids for src, pos, neg.
        """
        # In this file only have src, pos and neg; delete ref!
    ...
    def read_training_data(self, path: str) -> List[dict]:
        """
        Method that reads the training data (a csv file) and returns a list of samples.
    ...
    def predict_step(
        ...
        def _predict_forward(batch):
            ...
            distances = F.pairwise_distance(mt_sentemb, src_sentemb)
